...
from xlsx_operator import createRunningParamList, readXlsx
from xlsx_operator import createStudentInfoDis


# 传入xlsx表名，返回已填充数据的json数据
def body_create(xlsxName):
    # 从excel表中读取数据，生成工作表对象
    sheets = readXlsx(xlsxName)

    # 从工作表中读取学生数据，返回学生信息字典列表
    students = createStudentInfoDis(sheets[0])

    # 计算需要提交学生的人数
    studentsLenth = len(students)

    # 从工作表中读取运行参数，返回运行参数列表
    running_Param_List = createRunningParamList(sheets[1])

    if running_Param_List[5] == "申请长期留宿":
        # 待发送内容字符串(申请长期留宿)
        ...
        body_json["apply_data"]["contents"][2]["value"][
            "new_number"
        ] = running_Param_List[3]

        # 申请原因
        if running_Param_List[4] == "None":
            body_json["apply_data"]["contents"][3]["value"]["text"] = ""
        else:
            body_json["apply_data"]["contents"][3]["value"][
                "text"
            ] = running_Param_List[4]

        # 变更类型名称
        body_json["apply_data"]["contents"][4]["value"]["selector"]["options"][0][
            "value"
        ][0]["text"] = running_Param_List[5]

        # 变更类型opthon-id
        if running_Param_List[5] == "申请临时留宿":
            body_json["apply_data"]["contents"][4]["value"]["selector"]["options"][0][
                "key"
            ] = "option-528888885"
        elif running_Param_List[5] == "申请临时不留宿":
            body_json["apply_data"]["contents"][4]["value"]["selector"]["options"][0][
                "key"
            ] = "option-528888886"
        elif running_Param_List[5] == "申请取消长期留宿":
            body_json["apply_data"]["contents"][4]["value"]["selector"]["options"][0][
                "key"
            ] = "option-528888887"
        else:
    ...
    # 创建者id号-共有
    body_json["creator_userid"] = running_Param_List[0]

    # 所属部门id号-共有
    if running_Param_List[1] == "信息技术系":
        body_json["choose_party"]["partyid"] = "1688851641314945"
    elif running_Param_List[1] == "智能制造系":
        body_json["choose_party"]["partyid"] = "1688851641307308"
    elif running_Param_List[1] == "财经商贸系":
        body_json["choose_party"]["partyid"] = "1688851641307309"
    elif running_Param_List[1] == "公共基础部":
        body_json["choose_party"]["partyid"] = "1688851641307305"
    else:
        body_json["choose_party"]["partyid"] = ""

    # 所属部门名称-共有
    body_json["choose_party"]["party_name"] = running_Param_List[1]

    # 班级名称-共有
    body_json["apply_data"]["contents"][1]["value"]["text"] = running_Param_List[2]

    # 根据工作表最大行数，生成对应数量的字典
    children_dics = [dict(children_dic[0]) for i in range(studentsLenth)]

    # 由于批量生成的dics，内存空间黏连问题，需要把json对象先转json str，后再转json对象
    ...
    for children in children_dics:
        # 留宿变更名单--学生姓名
        children["list"][0]["value"]["text"] = students[0]["name"]
        # 留宿变更名单--学生手机
        children["list"][1]["value"]["new_number"] = students[0]["phone"]
        # 留宿变更名单--学生家长
        children["list"][2]["value"]["text"] = students[0]["parent"]
        # 留宿变更名单--学生家长手机
        children["list"][3]["value"]["new_number"] = students[0]["parentPhone"]
        # 留宿变更名单--学生宿舍
        children["list"][4]["value"]["text"] = students[0]["dormitory"]
        # 留宿变更名单--学生家庭住址
        children["list"][5]["value"]["text"] = students[0]["address"]
        students.pop(0)
...
if __name__ == "__main__":
    body_json = body_create("./students_info.xlsx")
